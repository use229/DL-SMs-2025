...
def load_data(filename):
    ...
    for i in range(m):
        for j in range(n1):
            lvs[i, j] = 1 if sheet1.cell(i, j).value > 0.5 else 0

    # Here we divide the data into two groups: 0-249 and 249-end
    group1 = lvs[:249]  # 1th group of data
    group2 = lvs[249:]  # 2th group of data

    # Extract training, validation, and test sets from each group
    tr_lvs_1, va_lvs_1, te_lvs_1 = split_data(group1)
    tr_lvs_2, va_lvs_2, te_lvs_2 = split_data(group2)

    # Combine the results
    combined_tr_lvs = np.vstack((tr_lvs_1, tr_lvs_2))
    combined_va_lvs = np.vstack((va_lvs_1, va_lvs_2))
    combined_te_lvs = np.vstack((te_lvs_1, te_lvs_2))

    return combined_tr_lvs, combined_va_lvs, combined_te_lvs

def split_data(data):
    m = data.shape[0]
    tr_lvs = data[:int(m * 0.90)]  # 90% of data for training
    va_lvs = data[int(m * 0.90):int(m * 0.95)]  # 5% of data for training
    te_lvs = data[int(m * 0.95):] # 5% of data for training
    return tr_lvs, va_lvs, te_lvs



def training_model():
    # Load data
    tr_lvs, va_lvs, te_lvs = load_data("..\\dataset\\image.xlsx")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Prepare training data
    train_inputs = torch.tensor(tr_lvs, dtype=torch.float32).to(device)
    train_targets = torch.tensor(tr_lvs, dtype=torch.float32).reshape(-1, 1, 50, 50).to(device)
    ...
    va_inputs = torch.tensor(va_lvs, dtype=torch.float32).to(device)
    va_targets = torch.tensor(va_lvs, dtype=torch.float32).reshape(-1, 1, 50, 50).to(device)

    te_inputs = torch.tensor(te_lvs, dtype=torch.float32).to(device)
    te_targets = torch.tensor(te_lvs, dtype=torch.float32).reshape(-1, 1, 50, 50).to(device)
